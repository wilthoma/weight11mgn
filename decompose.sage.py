...
# This file was *autogenerated* from the file decompose.sage
...
parser.add_argument("-rows", type=int, nargs='?',
                    help="Maximum number of rows in table.", default=_sage_const_100 )
parser.add_argument("-columns", type=int, nargs='?',
                    help="Maximum number of columns in table.", default=_sage_const_100 )
...
outtex = "\\begin{tabular}{|g|" + "|".join(["M" for i in range(nCols+_sage_const_1 )]) + "|} \\hline \\rowcolor{Gray}" +     " g,n & " + " & ".join([str(i)
                           for i in range(nCols+_sage_const_1 )]) + "\\\\ \n\\hline\n"
for g in range(nRows+_sage_const_1 ):
    outtex += str(g)
    for n in range(nCols+_sage_const_1 ):
